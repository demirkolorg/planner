...
export function QuickTaskModal({ isOpen, onClose }: QuickTaskModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Kısa bir gecikme sonrası kapat (kullanıcı sonucu görsün)
      setTimeout(() => {
        onClose()
      }, 1500)
          ...
          {analysis && !isAnalyzing && (
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="text-sm font-medium text-green-700 dark:text-green-300">Görev oluşturuldu!</span>
              </div>
              <div className="text-sm text-green-600 dark:text-green-400">
                <p><strong>Başlık:</strong> {analysis.title}</p>
                <p><strong>Öncelik:</strong> {analysis.priority}</p>
                {analysis.dueDate && (
                  <p><strong>Bitiş tarihi:</strong> {new Date(analysis.dueDate).toLocaleDateString('tr-TR')}</p>
                )}
                {analysis.tags.length > 0 && (
                  <p><strong>Etiketler:</strong> {analysis.tags.join(', ')}</p>
                )}
              </div>
            </div>
          )}
